...
func (server msgServer) SubmitFraudChallenge(goCtx context.Context, msg *types.MsgSubmitFraudChallenge) (*types.MsgSubmitFraudChallengeResponse, error) {
	...
	newAccountStates, err := replay.Replay(ctx, txsBz, msg.AccountStates, msg.Programs)
	if err != nil {
		fmt.Println("fails to replay solana txs")
		return nil, err
	}
	accountState := types.Account{}
	for _, newAccountState := range newAccountStates {
		if newAccountState.Pubkey == msg.FraudState {
			accountState = newAccountState
			fmt.Println("found fraud state")
			break
		}
	}
	accountStateHash, err := AccountToValue(accountState)
	if err != nil {
		fmt.Println("fails account to value")
		return nil, err
	}

	// check if the merkle root generated by the new account state and old merkle proof conflicts with the recorded merkle root
	msg.MerkleProof.Commitment = string(accountStateHash)
	if err := server.Keeper.Validate([]byte(merkleRoot), msg.MerkleProof); err != types.ErrInvalidateMerkleProof {
		fmt.Println("fails ErrInvalidateMerkleProof")
		return nil, types.ErrInvalidateMerkleProof
	}

	// TODO: charge gas fee if challenge fails

	fmt.Println("succeed to challenge")
